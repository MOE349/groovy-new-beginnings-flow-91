...
import { AlertTriangle, Camera, Edit2, Trash2, Plus } from "lucide-react";
          ...
          <TabsContent value="metering-events" className="tab-content-container">
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-4">Meter Reading Events</h3>
                  <ApiTable 
          ...
          <TabsContent value="scheduled-maintenance" className="tab-content-container">
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-6">
                <div>
                  <h3 className="text-lg font-semibold mb-4">PM Triggers</h3>
                  <ApiTable 
                    endpoint="/pm-automation/pm-settings"
                    filters={{ asset: id }}
                    columns={[
                      { key: 'name', header: 'Name' },
                      { key: 'interval_value', header: 'Every' },
                      { key: 'interval_unit', header: 'Unit' },
                ...
                <div>
                  <div className="bg-card border rounded-lg p-6">
                    <h4 className="text-lg font-semibold mb-4">Meter Reading Trigger</h4>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <Input 
                          ...
                          onChange={e => setMeterTriggerData(prev => ({ ...prev, name: e.target.value }))} 
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input 
                            ...
                            onChange={e => setMeterTriggerData(prev => ({ ...prev, interval_value: e.target.value }))} 
                            placeholder="Value"
                            className="flex-1"
                          />
                          <select 
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="text-sm font-medium mb-2 block">Starting at</label>
                        <Input 
                          type="number" 
                          value={meterTriggerData.start_threshold_value} 
                          onChange={e => setMeterTriggerData(prev => ({ ...prev, start_threshold_value: e.target.value }))} 
                          placeholder="Starting value"
                        />
                      </div>
                      <div>
                        <label className="text-sm font-medium mb-2 block">Create WO</label>
                        <div className="flex items-center gap-2">
                          <Input 
                            type="number" 
                            value={meterTriggerData.lead_time_value} 
                            onChange={e => setMeterTriggerData(prev => ({ ...prev, lead_time_value: e.target.value }))} 
                            placeholder="Lead time"
                            className="flex-1"
                          />
                          <span className="text-sm text-muted-foreground">units before</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <Button 
                        variant={meterTriggerData.is_active ? "default" : "outline"}
                      ...
                      onClick={async () => {
                        ...
                        try {
                          if (isEditMode && selectedItemId) {
                            ...
                            toast({
                              title: "Success",
                              description: "PM Trigger updated successfully!"
                            });
                          } else {
                            ...
                            toast({
                              title: "Success",
                              description: "PM Trigger created successfully!"
                            });
                          }
                          fetchPmTriggers();
                        } catch (error) {
                          toast({
                            title: "Error",
                            description: "Failed to save PM Trigger",
                            variant: "destructive"
                          });
                        }
                      }}
                    >
                      {isEditMode ? 'Update Trigger' : 'Save Trigger'}
                    </Button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ApiTable 
                    ...
                    tableId={`work-orders-${id}`}
                  />
                  <div className="mt-4 flex gap-2">
                    <Button onClick={() => setIsBacklogDialogOpen(true)}>
                      Add to Backlog
                    </Button>
              ...
              <div>
                <h3 className="text-lg font-semibold mb-4">PM Checklists</h3>
                <PMChecklistTabs assetId={id || ''} selectedPmId={selectedItemId} onNavigateBack={() => {}} />
              </div>
            </div>
          </TabsContent>
